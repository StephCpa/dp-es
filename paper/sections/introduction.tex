...
Applying DP to prompt optimization is non-trivial due to the discrete nature of text. Recent work like DP-OPT \cite{hong2024dpopt} uses histogram-based token generation with the exponential mechanism. Other approaches apply DP to in-context learning via private few-shot generation or nearest neighbor retrieval \cite{tang2024privacy,koskela2025differentially}, but they rely on additional retrieval or embedding machinery. While DP-OPT is effective on simple tasks, we identify critical limitations on complex reasoning: our analysis of 30 DP-OPT runs reveals a \textbf{63\% failure rate} on GSM8K (Appendix~\ref{sec:appendix:dpopt_failures}, Table~\ref{tab:dpopt_full_failures}), with systematic issues including template damage (destruction or partial loss) and suboptimal selection. This fragility stems from token-level myopia and non-backtracking search, which become catastrophic under tight privacy constraints.
...
\begin{enumerate}[leftmargin=*,itemsep=-0.3em]
    \item \textbf{Failure-mode diagnosis:} A 30-run analysis of DP-OPT on GSM8K identifying \emph{template damage} (placeholder loss, 26\% of failures) and \emph{noise-driven suboptimal selection} (74\%) as structural failure modes of token-level DP prompt construction, supported by concrete trajectory evidence (Section~\ref{sec:results}).
    ...
    \item \textbf{Empirical robustness:} 0/30 operational failures on GSM8K (vs.\ 19/30 for DP-OPT), with $\sim$9$\times$ lower standard deviation, plus strong or competitive results on MedQA, BANKING77, and Alpaca under a conservative $(\varepsilon{\leq}1,\delta{=}10^{-5})$ guarantee.
