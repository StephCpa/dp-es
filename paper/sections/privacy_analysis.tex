...
DP-ES inherits formal $(\varepsilon,\delta)$-DP from a \textit{Decoupled DP Evolution} view: mutations operate on public or previously privatized prompts, and only sampled-Gaussian evaluation accesses $\calD$. RDP accounting composes at most $TK$ score releases; both deterministic and Gumbel-smoothed selection are post-processing. The worst reported bound is $(\varepsilon{=}0.71,\delta{=}10^{-5})$, which we conservatively state as $\varepsilon\leq1$. Full derivation and reproducible accountant settings are in Appendix~\ref{sec:appendix:privacy_details}.
