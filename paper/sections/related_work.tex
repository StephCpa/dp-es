...
Evolution strategies (ES) \cite{rechenberg1973evolutionsstrategie} are derivative-free algorithms that maintain and evolve populations of candidate solutions. Prior non-private prompt optimizers, including EvoPrompt \cite{guo2024evoprompt} and PromptBreeder \cite{fernando2023promptbreeder}, already use LLM-driven mutation and population-based selection to optimize discrete prompts. DP-ES builds on this evolutionary-prompt-optimization line while introducing record-level DP through sampled-Gaussian scoring and post-processing selection. The population-based design provides natural robustness to noise-induced regressions---a critical advantage under tight privacy budgets where evaluation noise is substantial.
...
EvoPrompt & \texttimes & \checkmark & Evolutionary search \\
PromptBreeder & \texttimes & \checkmark & Evolutionary search \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Comparison of prompt optimization methods. DP-ES adds formal record-level DP to evolutionary optimization over discrete prompts.}
