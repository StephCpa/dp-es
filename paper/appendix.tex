...
\begin{itemize}
    \item Population size: $K=6$ prompts per generation; $M=2$ parents selected per round
    ...
    \item Selection: Gumbel-smoothed top-$M$ over privatized scores (default); deterministic top-$M$ is an alternative
    \item Accounting: uniform subsampling without replacement and RDP composition over at most $TK=18$ score releases, converted at $\delta=10^{-5}$
...
\noindent \textbf{Reproducibility:} We report run counts with each result (30 runs for the GSM8K stability analysis, five seeds for the selector ablation, and three seeds for the validation studies). Code, prompts, accountant inputs, and evaluation scripts accompany the final paper.
...
\begin{align*}
  Z_t &= \mathcal{Q}_t(\calD, \calP_t), &
  \mathcal{S}_t &= \mathrm{Sel}_t(Z_t), \\
  \calP_{t+1} &= G_t(\mathcal{S}_t, U_t). &&
\end{align*}

\noindent where $\mathcal{Q}_t$ releases $K$ sampled-Gaussian scores in each executed round, $\mathrm{Sel}_t$ retains $M$ parents from those privatized scores, and $G_t$ mutates selected prompts using only public randomness $U_t$. Both $\mathrm{Sel}_t$ and $G_t$ are post-processing. Thus the total privacy cost is that of at most $TK$ adaptively composed sampled-Gaussian score releases.


\begin{proof}[Proof sketch]
\textbf{Step 1 (Privacy source).} At each round $t$, only $\mathcal{Q}_t$ touches $\calD$. Each candidate score uses a uniformly sampled batch of size $b$, clips per-record utility to $[0,1]$, averages it, and adds Gaussian noise with standard deviation $z/b$.
...
\noindent \textbf{Step 3 (RDP composition).} We apply the without-replacement subsampling bound of \citet{wang2019subsampled} to each score release, add RDP across at most $TK$ adaptive releases, and convert to $(\varepsilon,\delta)$-DP. Post-processing then gives the same guarantee for the released final prompt.
...
For a batch mean with clipped per-record utility in $[0,1]$, replace-one sensitivity is $\Delta_B=1/b$. Adding noise $\mathcal{N}(0,(z/b)^2)$ is a Gaussian mechanism with noise multiplier $z$. We apply uniform subsampling without replacement at rate $q=b/n$, compose at most $TK$ mechanisms in RDP, and convert at $\delta=10^{-5}$ using AutoDP \cite{autodp}. Reported runs use $b=10$ and $z=10$. With $K=6$ and $T=3$, the resulting worst-case bound is given in Table~\ref{tab:rdp_bounds}.
...
\midrule
All reported sampled-score runs & 200 & 0.05 & $\leq0.71$ \\
\bottomrule
\end{tabular}
\caption{Recomputed worst-case privacy bound for at most 18 sampled-Gaussian score releases in the reported DP-ES configuration. We report the common conservative ceiling $\varepsilon\leq1$.}
\label{tab:rdp_bounds}
\end{table*}

\noindent\textbf{Gumbel-smoothed selection.}
For smoothing only, we add independent Gumbel noise to each privatized score before ranking, with $\beta$ fixed independently of $\calD$:
\begin{equation}
\hat{s}_p = \tilde{s}_p + \operatorname{Gumbel}(0,\beta).
\end{equation}
Because this operation is a function only of already privatized scores and fresh randomness, it is post-processing for every $\beta\geq0$. Deterministic top-$M$ is the $\beta=0$ special case. Table~\ref{tab:em_ablation} compares their utility.
...
\noindent Main results appear in Section~\ref{sec:results}. This appendix provides efficiency comparisons, DP-OPT stability and trajectory analysis, local validation (including PromptDPSGD), ablations, accounting details, and implementation diagnostics.
...
GSM8K & 14.60 hours & \textbf{7.37 hours} & \textbf{1.98$\times$} \\
MedQA & 3.24 hours & \textbf{1.87 hours} & \textbf{1.73$\times$} \\
\bottomrule
\end{tabular}
...
\noindent Across 30 GSM8K runs, DP-OPT achieves $49.5{\pm}28.5\%$, whereas DP-ES achieves $88.1{\pm}3.2\%$. We use these distributional statistics directly rather than imposing a post-hoc accuracy threshold to label runs as failures. Table~\ref{tab:dpopt_stability} summarizes the aggregate statistics and representative trajectory diagnostic. The main-text visualization (Figure~\ref{fig:dpopt_failures_summary}) reproduces an actual DP-OPT trajectory and illustrates two mechanisms consistent with the observed instability: prompt-template drift during token-level construction and irreversible propagation of noisy ranking decisions.
...
\noindent \textbf{Why DP-ES Improves Stability:}

\begin{itemize}
    \item \textbf{Prompt-level generation:} Mutations operate on complete prompts, preserving global structure
    \item \textbf{Population diversity:} Even if one candidate is degraded by noise, others can compensate
...
\noindent To check whether the main task-dependent trend transfers beyond the API model, we evaluate four methods with a local Qwen2.5-7B model \cite{qwen2.5} under the common verified $\varepsilon\leq1$ setting. Table~\ref{tab:local_validation} reports the results.
...
\bottomrule
\end{tabular}
\caption{\textbf{Local Qwen2.5-7B validation} at the common $\varepsilon\leq1$ setting (three seeds; mean $\pm$ std). Non-DP is a reference rather than an upper bound because the methods use different search procedures.}
...
\noindent Table~\ref{tab:em_ablation} reports the five-seed comparison at the default setting. Gumbel-smoothed and deterministic top-$M$ are statistically similar (88.8\% vs.\ 88.6\%) and have identical privacy accounting because both post-process privatized scores. We use smoothing by default but do not claim a consistent variance advantage.
...
Mutations consume zero budget (Proposition~\ref{prop:mutation-free}). Each executed iteration releases $K$ sampled-Gaussian scores. Applying the without-replacement RDP amplification bound to each release, composing the resulting RDP guarantees over at most $TK$ adaptive calls, and converting at the target $\delta$ yields Table~\ref{tab:rdp_bounds}. Selection and subsequent mutation are post-processing, so the final prompt inherits the same $(\varepsilon,\delta)$ guarantee.
