...
For a batch mean with clipped per-record utility in $[0,1]$, replace-one sensitivity is $\Delta_B=1/b$. Adding noise $\mathcal{N}(0,(z/b)^2)$ is a Gaussian mechanism with noise multiplier $z$. We apply uniform subsampling without replacement at rate $q=b/n$, compose $TK$ mechanisms in RDP, and convert at $\delta=10^{-5}$ using AutoDP \cite{autodp}. Reported runs use $b=10$ and $z=10$. With $K=6$ and $T=3$, the resulting bound is given in Table~\ref{tab:rdp_bounds}.
...
\bottomrule
\end{tabular}
\caption{Recomputed privacy bound for the 18 sampled-Gaussian score releases used in reported DP-ES experiments. We report the common conservative ceiling $\varepsilon\leq1$.}
