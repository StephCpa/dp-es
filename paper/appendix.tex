...
\textbf{DP-OPT} \cite{hong2024dpopt}: State-of-the-art DP prompt optimization using histogram-based token generation with the exponential mechanism. We use the official implementation \footnote{\url{https://github.com/VITA-Group/DP-OPT}}.
...
\noindent \textbf{Non-DP}: Standard prompt optimization without privacy constraints, implemented using the TextGrad\footnote{\url{https://github.com/zou-group/textgrad}} \cite{yuksekgonul2024textgrad}
 method. This provides an upper bound on achievable performance.
...
\subsection{DP-OPT Detailed Failure Analysis}
\label{sec:appendix:dpopt_failures}

\noindent We conducted a comprehensive analysis of 30 DP-OPT experiments on GSM8K and identified 19 anomalous runs (63\% failure rate) exhibiting systematic failure patterns. Two modes dominate among the failures: \textbf{template damage} (destruction or partial loss; 5/19, 26\%, typical accuracy $\leq$25\%) and \textbf{suboptimal selection} (14/19; 74\%, typical accuracy 35--55\%); in contrast, DP-ES shows \textbf{0/30} observed failures. Table~\ref{tab:dpopt_full_failures} categorizes these failures by severity. The main-text visualization (Figure~\ref{fig:dpopt_failures_summary}) reproduces an actual DP-OPT trajectory alongside the aggregate failure counts.
...
\begin{tabular}{llcc}
\toprule
\textbf{Severity} & \textbf{Description} & \textbf{Count} & \textbf{Acc Range} \\
\midrule
Severe & Template destroyed & 2 & 0.0\% \\
Moderate & Partial template loss & 3 & 12.5-25.0\% \\
Minor & Suboptimal selection & 14 & 35.0-55.0\% \\
\midrule
\textbf{Total Failures} & & \textbf{19/30} & -- \\
\textbf{DP-ES Failures} & & \textbf{0/30} & -- \\
\bottomrule
\end{tabular}
\caption{\textbf{Full DP-OPT Failure Breakdown.} 14 of 19 failures share suboptimal selection; template damage (destruction or partial loss) is the most severe mode.}
\label{tab:dpopt_full_failures}
\end{table*}
...
\begin{itemize}
    \item Delete critical placeholders like \texttt{\{question\}} mid-generation
...
\noindent \textbf{Suboptimal selection.} DP noise can reverse the ordering of candidates with similar utility. DP-OPT commits to each token-level decision without backtracking, so an early rank inversion can permanently redirect construction. DP-ES does not eliminate rank inversions; instead, its population retains multiple complete hypotheses and can recover in later rounds. The 14 runs in this category end in the 35--55\% accuracy cluster without complete placeholder destruction.
\vspace{0.5em}
...
This analysis explains the 38.6 pp performance gap on GSM8K: DP-OPT's algorithmic fragility causes frequent catastrophic failures, while DP-ES's population-based design provides robustness.
