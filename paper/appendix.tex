...
\noindent where $M_t$ releases $K$ sampled-Gaussian scores in each executed round, $\mathrm{Sel}_t$ selects from those privatized scores, and $G_t$ mutates selected prompts using only public randomness $U_t$. Both $\mathrm{Sel}_t$ and $G_t$ are post-processing. Thus the total privacy cost is that of at most $TK$ adaptively composed sampled-Gaussian score releases.
